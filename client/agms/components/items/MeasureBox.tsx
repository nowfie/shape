...
const MeasureBox = ({item}:MeasureBoxProps) => {
  let Icon = item.icon
  ...
  return (
    <Link href={`/detail/measurement/${3}`} className=' w-full'>
      <View className={` ${statStyle} p-4 xs:p-5 rounded-3xl w-full gap-4`}>
        ...
        <View className=' flex-row gap-3 items-baseline'>
            ...
            <Text type='para' className='paragraph'> cm</Text>
        </View>
    </View>
    </Link>
  )
